...
abstract class RequestService extends AbstractService
{
     ...
     *
     * Paremeters key are class setter transform from camel case to undescore string (@see FnacApiClient\Toolbox\String )
     * Example :
     ...
     * {@inheritdoc}
     */
    final public function denormalize(NormalizerInterface $normalizer, $data, $format = null)
    {
    ...
    public function initParameters(array $parameters)
    {
        foreach ($parameters as $getter => $param) {
            $setter = 'set' . String::toCamelCase($getter);
