...
class PricingQuery extends Authentified
{
    ...
    {
        $data = parent::normalize($normalizer, $format, $properties);
        ...
        if ($this->product_reference->count() > 1) {
            foreach ($this->product_reference as $product_reference) {
                $data['product_reference'][] = $product_reference->normalize($normalizer, $format, $properties);
            }
        } elseif ($this->product_reference->count()) {
            $data['product_reference'] = $this->product_reference[0]->normalize($normalizer, $format, $properties);
        }
