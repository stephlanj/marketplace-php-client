...
class Order extends Entity
{
        ...
        if ($this->orders_detail->count() > 0) {
            if ($this->orders_detail->count() > 1) {
                ...
                foreach ($this->orders_detail as $order_detail) {
                    $data['order_detail'][] = $order_detail->normalize($normalizer, $format, $properties);
                }
            } else {
                $data['order_detail'] = $this->orders_detail[0]->normalize($normalizer, $format, $properties);
            }
        }
        return $data;
    }
    ...
    public function denormalize(NormalizerInterface $normalizer, $data, $format = null)
    {
        ...
        $this->shipping_address->denormalize($normalizer, $data['shipping_address'], $format);
        $this->billing_address = new Address();
        $this->billing_address->denormalize($normalizer, $data['billing_address'], $format);
        ...
        if (isset($data['order_detail'][0])) {
            foreach ($data['order_detail'] as $order_detail) {
                ...
                $tmpObj->denormalize($normalizer, $order_detail, $format);
                $this->orders_detail[] = $tmpObj;
            }
        } elseif (!empty($data['order_detail'])) {
            $tmpObj = new OrderDetail();
            $tmpObj->denormalize($normalizer, $data['order_detail'], $format);
            $this->orders_detail[] = $tmpObj;
        }
    }
